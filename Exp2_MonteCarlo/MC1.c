...
void doMaster()
{
	...
	while (RUN > k)
	{
		...
		if (recvBuf >= 0)
		{						  // This section will record the results of each job
		...
		{ // This section will generate a new job
...
void doSlave()
{
	...
	while (RUN / (size - 1) > k)
	{ // size-1 is the total number of slave
		...
		{ // test x^2+y^2, test how many dots are inside the circle
